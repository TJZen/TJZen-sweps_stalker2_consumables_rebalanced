if CLIENT then 
    SWEP.WepSelectIcon = surface.GetTextureID( "vgui/hud/vgui_stalker2_canned" )
	SWEP.BounceWeaponIcon = true 
    SWEP.DrawWeaponInfoBox = true
end

SWEP.PrintName = "Canned Food"
...
SWEP.ViewModel = "models/weapons/sweps/stalker2/canned/v_item_canned.mdl"
SWEP.WorldModel = "models/weapons/sweps/stalker2/canned/w_item_canned.mdl"
SWEP.UseHands = true
...
local INI_AUX = false
local ID_WEAPON = "weapon_stalker2_canned"
local ID_PRIMARYAMMO = "canned"
...
function SWEP:InitializeConsumable()
    ...
    timer.Simple(SequenceDuration * 0.59, function() -- Call item effects.
	...
	timer.Simple(SequenceDuration * 0.83, function() -- Call item effects.
        if IsValid(owner) and owner:Alive() then
			self:Heal(owner)
        end
    end)
...
function SWEP:Heal(owner)
	if IsValid(owner) and owner:GetActiveWeapon():GetClass() == ID_WEAPON then
		...
		if INI_SEF == true and SERVER then
			owner:ApplyEffect("Healing", 1.3, 1, 0.25)
		end
