if CLIENT then 
    SWEP.WepSelectIcon = surface.GetTextureID( "vgui/hud/vgui_stalker2_energy" )
	SWEP.BounceWeaponIcon = true 
    SWEP.DrawWeaponInfoBox = true
end

SWEP.PrintName = "Energy"
SWEP.Author = "Craft_Pig"
SWEP.Purpose = 
[[
Heals 10 HP Over 5s
Grants 20 Haste for 40s
]]
...
SWEP.ViewModel = "models/weapons/sweps/stalker2/energy/v_item_energy.mdl"
...
local INI_AUX = false
local ID_WEAPON = "weapon_stalker2_energy"
local ID_PRIMARYAMMO = "energy"
...
function SWEP:Heal(owner)
	if IsValid(owner) and owner:GetActiveWeapon():GetClass() == ID_WEAPON then
		...
		if INI_SEF == true and SERVER then
			 owner:ApplyEffect("Haste", 40, 20)
			 owner:ApplyEffect("Healing", 5, 1, 0.5)
		end
		
		if INI_AUX == true then
			AUXPOW:SetPower(owner, math.min(AUXPOW:GetPower(owner) + 0.8, 1));
		end
